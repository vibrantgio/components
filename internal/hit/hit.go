...
// 24 dp Comfortable, 19 dp Compact — but the WCAG 2.5.5 pointer target never
...
// component's bounds; where the slop of neighbouring controls overlaps, the
...
// returned dimensions are w's own visual dimensions; w draws at the layout
// origin as usual.
func Extend(gtx layout.Context, minPx int, lay func(layout.Context, layout.Widget) layout.Dimensions, w layout.Widget) layout.Dimensions {
	...
	call := macro.Stop()
	// Shift back so the visual sits at the layout origin; the hit slop

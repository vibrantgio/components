...
// thumbSurfaces are the levels an overlay bar rides: the window's own page in
...
// TestTheThumbClearsItsFloorOnEverySurfaceItRides asserts the floors where they
...
func TestTheThumbClearsItsFloorOnEverySurfaceItRides(t *testing.T) {
			...
			if step > foregroundStep {
				...
				if clears(scheme.c, shallower, st.floor) {
					t.Errorf("%s %s: step %d clears the floor at coverage %d, so step %d is deeper than the thumb needed",
						scheme.name, st.name, step-100, st.foreground.A, step)
				}
			}
		}
	}
...
func TestTheTwoStatesComeFromOneRecipe(t *testing.T) {
	for _, scheme := range []struct {
		...
		if stepOf(t, scheme.c, s.ThumbHoverColor) < stepOf(t, scheme.c, s.ThumbColor) {
...
// stepOf returns the neutral-ramp step `foreground` was taken from, failing
// the test if it was taken from anywhere else: the thumb is a ramp colour at some
// coverage, and a hand-mixed one would slip past every ratio above.
func stepOf(t *testing.T, c tokens.ColorTokens, foreground color.NRGBA) int {
	t.Helper()
	for step := 100; step <= 900; step += 100 {
		if stepColor := c.Ramps.Neutral.Step(step); stepColor.R == foreground.R && stepColor.G == foreground.G && stepColor.B == foreground.B {
			return step
		}
	}
	t.Fatalf("thumb colour %v is not a step of the neutral ramp", foreground)
	return 0
}

...
// drawing a weaker foreground than its role names is the one this gate was
...
// label on it is the foreground that role names, and the two measure at or
// above WCAG AA for body text.
//
// It is a render-side gate and not a token-side one: a token gate checks the
// derivation, not what the page painted, so it cannot confirm or refute a chip
// drawn with the wrong foreground. The question is asked of the pixels
...
// toward the token's foreground. A chip painting a foreground its role does
// not name lands short of the floor; a chip painting the right one lands
...
// page's own fill between them. It returns each run as a half-open x range.
// Reading the row rather than assuming a pitch is what lets the test measure
// the chip it names when a long role name widens its column.

...
// The corner readers are components/golden's: a capture rendered onto a clear
// window carries the rasterizer's coverage in its alpha channel, and more
// than one package reads a corner off one. They are named here so the
// readings below stay the sentences they were.
var (
	coverageRow  = golden.CoverageRow
	leadingEdge  = golden.LeadingEdge
	trailingEdge = golden.TrailingEdge
	fitCorner    = golden.FitCorner
)

...
| Package | |
| --- | --- |
| `alert` | The status signal for a situation: a rounded box — a leading status glyph, a title and an arbitrary body slot — standing in the page flow until the situation resolves. It stands on the content's fill inside the platform's separator hairline, its title in the platform's label colour, and the status is carried by the glyph alone, in the platform's system colour for it: systemGreen, systemOrange, systemRed, systemBlue. Info is systemBlue and never the accent. It holds words about the situation, never a control: an action on the situation stands beside it. |
...
| `toast` | The status signal for an event: a small surface floating at level 2 that appears when a notification is raised and leaves by itself after a set time. Level 2 is where it is placed, not what it is filled with: a floating surface on this platform is the window's own plane, so that is its fill and the platform's label colour its message. Its status speaks through the leading edge, in the platform's system colour for it, and never through the fill. It draws one toast and no queue; the column that receives notifications, places, stacks and times them is [`patterns/notifications`](https://github.com/vibrantgio/patterns), and the cast shadow that says a toast floats belongs to that placement. |
| `tooltip` | The signal naming a control or explaining another signal on demand: a small annotation floating at level 3 beside its trigger, appearing by itself after a short delay on hover or focus and leaving when they do. Level 3 is where it is placed, not what it is filled with: it is the window's own plane inside the platform's separator hairline, with its text in the platform's label colour, the same in both appearances, so it reads as an annotation about the thing rather than as a panel. It holds text only, never a control: anything the user must operate is the job for a popover. Arbitration is frame state rather than a bus — `Props.Arbiter` names the set, one per window, and a tooltip is visible exactly while it holds that set's top, so the claim a finished dwell makes *is* the previous tooltip's dismissal. A nil `Arbiter` gets the tooltip one of its own, so sharing one is the explicit act. `DefaultDelay` resolves from the token motion scale's `DurXSlow` stop (500 ms), and the live form re-times from the theme's `Motion` observable. |
